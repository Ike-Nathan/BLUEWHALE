# Bluewhale Demo (Flutter)

A production-grade reference implementation of the `bluewhale_core`, demonstrating best practices for deposit routing in Stellar wallets and exchanges.
...
## Live Demo
🚀 [View Live Web Demo](https://redishfish.github.io/bluewhale-demo) *(Placeholder)*
...
1.  **Correctness**: The "BigInt-safe" chip in the Receive panel isn't just a label—it's backed by `bluewhale_core`'s core implementation that uses `BigInt` for all ID operations, ensuring that a user ID like `9007199254740993` (2^53 + 1) is never corrupted.
...
[bluewhale_core on pub.dev](https://pub.dev/packages/bluewhale_core)
